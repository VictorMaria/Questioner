...
describe('User', function () {
  ...
  before(function (done) {
    var user1 = void 0;
    var users = [user1];
    api.post('/api/v1/users').set('Accept', 'application/json').send({
      firstname: "John",
      lastname: "Greer",
      othername: "JG",
  ...
  it('should return a 200 response', function (done) {
    api.get('/api/v1/users/1').set('Accept', 'application/json').expect(200, done);
  });

  // Tests for an object with key-value pairs when fetching a specific user
  it('should be an object with keys and values', function (done) {
    api.get('/api/v1/users/1').set('Accept', 'application/json').expect(200).end(function (err, res) {
  ...
  it('getUsers() should return a 200 response', function (done) {
    api.get('/api/v1/users').set('Accept', 'application/json').expect(200, done);
  });
});
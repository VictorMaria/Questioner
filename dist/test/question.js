...
describe('Question', function () {
  ...
  before(function (done) {
    var question1 = void 0;
    var questions = [question1];
    api.post('/api/v1/questions').set('Accept', 'application/json').send({
      createdBy: 1,
      meetup: 1,
  ...
  it('should return a 200 response', function (done) {
    api.get('/api/v1/questions/1').set('Accept', 'application/json').expect(200, done);
  });

  // Tests for an object with key-value pairs when fetching a specific question
  it('should be an object with keys and values', function (done) {
    api.get('/api/v1/questions/1').set('Accept', 'application/json').expect(200).end(function (err, res) {
  ...
  it('getQuestions() should return a 200 response', function (done) {
    api.get('/api/v1/questions').set('Accept', 'application/json').expect(200, done);
  });
});